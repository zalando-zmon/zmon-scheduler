...
public class EntityAdapterRegistry extends SourceRegistry<EntityAdapter> {
    ...
    public EntityAdapterRegistry(SchedulerConfig config, MetricRegistry metrics) {
        ...
        if(config.entity_service_url() !=null && !config.entity_service_url().equals("")) {
            EntityServiceAdapter e = new EntityServiceAdapter(config.entity_service_url(), config.entity_service_user(), config.entity_service_password(), config.entity_service_token(), metrics);
            register(e);
        }
    ...
    public EntityAdapterRegistry(SchedulerConfig config, ZalandoConfig zConfig, MetricRegistry metrics) {
        ...
        if(zConfig.entityservice != null && zConfig.entityservice.url != null) {
            EntityServiceAdapter e = new EntityServiceAdapter(zConfig.entityservice.url, zConfig.entityservice.user, zConfig.entityservice.password, zConfig.entityservice.token, metrics);
            register(e);
        }
