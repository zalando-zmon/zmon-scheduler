...
public class CheckSourceRegistry extends SourceRegistry<CheckSource> {
    ...
    public CheckSourceRegistry(SchedulerConfig config, final TokenWrapper tokens) {
        DefaultCheckSource source = new DefaultCheckSource("check-source", config.controller_url()+ (config.urls_without_rest() ? "" : "/rest") + "/api/v1/checks/all-active-check-definitions", tokens);
        register(source);
    }
    ...
    public CheckSourceRegistry(ZalandoCheckConfig config, SchedulerConfig schedulerConfig) {
        ...
        if(config.controller()!=null && config.controller().getUrl()!=null && !"".equals(config.controller().url())) {
            ...
            DefaultCheckSource source = new DefaultCheckSource(conf.name(), conf.url(), null);
            register(source);
        }
    }
}
